...
int32_t proc_create(void (*func)(void))
{
    ...
    void *stack = heap_alloc(PROC_STACK_SIZE);
    if (stack == NULL)
        return -1;
...
int proc_terminate(int32_t pid)
{
    ...
    {
        heap_free(proctab[pid].stack_base);
    }
...
{
    if (!valid_pid(dst_pid))
        return -1;
    if (proctab[dst_pid].state == PR_TERMINATED)
        return -1;
